...
import { AVATAR_EMOJIS, PLAYER_COLORS, MAX_PLAYERS, MIN_PLAYERS_GROUP } from '../constants';

type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'AddPlayers'> };

const AVATARS: AvatarType[] = ['boy', 'girl', 'man', 'woman', 'lion', 'tiger', 'robot', 'car', 'ball', 'star'];

export default function AddPlayersScreen({ navigation }: Props) {
  const { t } = useTranslation();
  const { pendingPlayers, addPlayer, removePlayer, settings } = useGameStore();
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.avatarRow}>
            {AVATARS.map(a => (
              <TouchableOpacity
                ...
                onPress={() => setAvatar(a)}>
                <Text style={styles.avatarEmoji}>{AVATAR_EMOJIS[a]}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
            ...
            {PLAYER_COLORS.map((c, i) => (
              <TouchableOpacity
                ...
                style={[styles.colorDot, { backgroundColor: c },
                  colorIdx % PLAYER_COLORS.length === i && styles.colorSelected]}
        ...
        {pendingPlayers.map(p => (
          <View key={p.id} style={[styles.playerCard, { borderColor: p.color }]}>
            <View style={[styles.playerAvatar, { backgroundColor: p.color + '33' }]}>
              <Text style={styles.playerEmoji}>{AVATAR_EMOJIS[p.avatar]}</Text>
            </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          ...
          onPress={() => canNext && navigation.navigate('AgeGroupSelect')}
          disabled={!canNext}>
...
const styles = StyleSheet.create({
  ...
  colorDot: {
    ...
    marginRight: 8, borderWidth: 3, borderColor: 'transparent',
  },
  ...
  addBtnText: { fontSize: 16, fontWeight: 'bold', color: Colors.text },
  playerCard: {
    backgroundColor: Colors.backgroundCard,
    borderRadius: 14,
    padding: 14,
  ...
  playerName: { flex: 1, fontSize: 17, fontWeight: '600', color: Colors.text },
  removeBtn: { padding: 6 },
  removeText: { fontSize: 18, color: Colors.error },
  hint: { textAlign: 'center', color: Colors.textMuted, fontSize: 14, marginTop: 8 },
  footer: {
